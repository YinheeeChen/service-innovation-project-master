...
from img_log import save_image, get_image
from record import getCoord
...
MODEL_NAME = None
SAVE_NAME = 'generation-13-eval-reward-94.04.model'
TRAIN = True
AGENT_HIST = 4
GAMMA = 0.99
...
def run():
    ...
    while frame_count < TOTAL_FRAMES:
        ...
        # ball_x, ball_y, panel_x, panel_y = 0,0,0,0
        # # print(f"count: {frame_count} Ball is at ({ball_x}, {ball_y}), Panel is at ({panel_x}, {panel_y})")
        # save_image(frame, IMG_DIR)
        terminal = False
        while not terminal:
            if TRAIN:
                if STATE_DEBUG:
            ...
            # ball_x, ball_y, panel_x, panel_y = getCoord(new_frame)
            # print(f"count: {frame_count} Ball is at ({ball_x}, {ball_y}), Panel is at ({panel_x}, {panel_y})")
            # save_image(new_frame, IMG_DIR)
            if TRAIN:
                # print(frame.shape)
...
def trained_model():
    env = Game(key=GAME_NAME)
    env.run_human_mode('episode-8166-reward-403.0.npy')
